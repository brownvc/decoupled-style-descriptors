...
Additional code development by Eva Schiller and Brayden Goldstein-Gelb.
...
If you would like to get started with the model without any code, there is an [online demo](https://huggingface.co/spaces/brayden-gg/decoupled-style-descriptors) demonstrating handwriting interpolation, character interpolation, and MDN sampling.

![Online Demo](example_outputs/online_demo.gif)

The outputs can be saved as an SVG file and easily imported into the program of your choice. The demo can also be run locally by running the `app.py` file and visiting the link generated by the program.
